...
class TestLevelBudgetPlan:
    ...
    def test_bpw_targets_for_level_returns_none_for_minimal(self):
        assert _bpw_targets_for_level(2) is None
    ...
    def test_budget_plan_oq2_not_enabled(self):
        assert 2 not in _OQ_BPW_TARGETS
    ...
    def test_oq2_no_budget_plan_boost(self):
        """oQ2 should use minimal protection, NOT budget plan boosts."""
        # Simulate what entry points do: oQ2 not in _OQ_BPW_TARGETS → empty boost_map
        assert _bpw_targets_for_level(2) is None
        config = {
            "num_hidden_layers": 32,
        ...
        module = MagicMock(spec=[])
        # lm_head should get bits(6) from minimal protection, not budget plan
        result = universal_quant_predicate("lm_head", module, config, 2)
        assert isinstance(result, dict)
        assert result["bits"] == 6

    def test_oq2_boost_map_stays_empty(self):
        """oQ2 should not generate any boost_map entries."""
        # _bpw_targets_for_level(2) is None → plan building should be skipped
        assert _bpw_targets_for_level(2) is None
        # If someone still calls _build_quant_plan for oQ2 with target 4.6,
        # it would generate boosts — this is why entry points must guard it
        assert 2 not in _OQ_BPW_TARGETS
